...
int main(int argc, char **argv) {
	...
	// Deterministic initialization: value = x + y + z (as double)
	for (int k = 0; k < NZ; ++k)
	for (int j = 0; j < NY; ++j)
	for (int i = 0; i < NX; ++i) {
	...
	// Time-stepping: simple 7-point stencil (center + 6 face neighbors)
	...
	// Compute a simple checksum to have deterministic output
	double sum = 0.0;
	for (size_t idx = 0; idx < n; ++idx) sum += grid[idx];

	// Ensure results directory exists and write CSV
	...
	if (f) {
		fprintf(f, "run,time_seconds,NX,NY,NZ,STEPS,checksum\n");
		fprintf(f, "serial,%0.9f,%d,%d,%d,%d,%.12f\n", elapsed, NX, NY, NZ, STEPS, sum);
		fclose(f);
	} else {
		fprintf(stderr, "Failed to write CSV: %s\n", strerror(errno));
	}

	// Also print TIME to stdout for scripts to parse
	printf("TIME: %0.9f\n", elapsed);
